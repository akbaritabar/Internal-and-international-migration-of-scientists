## File name to use in search: prepare_data_for_mapping_disaggregate_by_author_attributes.py ##
...
parser.add_argument("-dis", "--DISAGGREGATION", help = "Author attribute to disaggregate migration with", type = str, required = True)
...
# ============================
#### Disaggregate mapping data by discipline, academic age, or productivity  ####
# ============================

# NOTE, this procedure at the end exports a LONG format table with all categories of the chosen attribute to disaggregate, in the plotting script, this is considered and each category is exported to its own figure/map.
...
mcj_data = mcj_data[['region', 'year', DISAGGREGATION_COLUMN,
                     'country_2letter_code','geonames_admin1_code', 'geonames_admin1_ascii_name', 'continent_code',
...
nmr_INT_TIME_SPAN_region = (
    ...
    [['iso_a3', 'region', DISAGGREGATION_COLUMN, 'in_y_flow_INT', 'out_y_flow_INT', 'y_pop_INT']]
...
nmr_IN_TIME_SPAN_region = (
    ...
    .groupby(['iso_a3', 'region', DISAGGREGATION_COLUMN])
    # add grouping columns that will be used in join
    [['iso_a3', 'region', DISAGGREGATION_COLUMN, 'in_y_flow_IN', 'out_y_flow_IN', 'y_pop_IN']]
...
nmr_INT_IN_TIME_SPAN_region = (
    ...
    .groupby(['iso_a3', 'region', DISAGGREGATION_COLUMN])
    # add grouping columns that will be used in join
    [['iso_a3', 'region', DISAGGREGATION_COLUMN, 'in_y_flow_INT', 'out_y_flow_INT', 'in_y_flow_IN', 'out_y_flow_IN', 'y_pop_INT']]
...
mei_INT_IN_TIME_SPAN_region = (
    ...
    .groupby(['iso_a3', 'region', DISAGGREGATION_COLUMN])
    # add grouping columns that will be used in join
    [['iso_a3', 'region', DISAGGREGATION_COLUMN, 'in_y_flow_INT', 'out_y_flow_INT', 'in_y_flow_IN', 'out_y_flow_IN']]
...
mcj_data_joined = (mcj_data
            .merge(nmr_INT_TIME_SPAN_region, on=['iso_a3', 'region', DISAGGREGATION_COLUMN], how='left')
            .merge(nmr_IN_TIME_SPAN_region, on=['iso_a3', 'region', DISAGGREGATION_COLUMN], how='left')
            .merge(nmr_INT_IN_TIME_SPAN_region, on=['iso_a3', 'region', DISAGGREGATION_COLUMN], how='left')
