## File name to use in search: prepare_migration_data_v2_scaled_up.py ##
...
international_province['out_y_flow'] = international_province.groupby(['move_year', 'from'])['author_id'].transform('nunique')
international_province['in_y_flow'] = international_province.groupby(['move_year', 'to'])['author_id'].transform('nunique')

# drop duplicates & join with yearly population data
international_province_out = international_province[['from', 'move_year', 'out_y_flow']].drop_duplicates()
international_province_in = international_province[['to', 'move_year', 'in_y_flow']].drop_duplicates()
international_province_nmr = international_province_in.merge(international_province_out, how='outer', left_on=['to', 'move_year'], right_on=['from', 'move_year'])
...
international_province_nmr_to_join = region_y_pop.merge(international_province_nmr, right_on=['move_year', 'to'], left_on=['pubyear', 'geonames_admin1_code'], how='left')
...
internal_province['out_y_flow'] = internal_province.groupby(['move_year', 'from'])['author_id'].transform('nunique')
internal_province['in_y_flow'] = internal_province.groupby(['move_year', 'to'])['author_id'].transform('nunique')

# drop duplicates & join with yearly population data
internal_province_out = internal_province[['from', 'move_year', 'out_y_flow']].drop_duplicates()
internal_province_in = internal_province[['to', 'move_year', 'in_y_flow']].drop_duplicates()
internal_province_nmr = internal_province_in.merge(internal_province_out, how='outer', left_on=['to', 'move_year'], right_on=['from', 'move_year'])
...
# join with yearly population of scholars
internal_province_nmr_to_join = region_y_pop.merge(internal_province_nmr, right_on=['move_year', 'to'], left_on=['pubyear', 'geonames_admin1_code'], how='left')
# calculate yearly NMR (yearly net_flow / yearly pop) * 1000
...
without_threshold = (international_province_nmr_to_join
                     [['to', 'move_year', 'in_y_flow', 'out_y_flow', 'net_y_flow', 'y_pop', 'nmr','abs_inout', 'sum_inout', 'pop_y_in_and_out']]
                     .rename(columns={
                            'to':'region',
                            'move_year':'year',
                            'y_pop':'y_pop_INT',
